...
export const FrontPageManager: React.FC<FrontPageManagerProps> = ({
  ...
  return (
    <div className="space-y-12 pb-12">
      ...
      <div className="pb-6 border-b border-white/10 font-sans">
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
          <div>
            <div className="flex items-center gap-2 text-xs font-sans uppercase tracking-wider text-neutral-400 mb-1 font-semibold">
            ...
            <p className="text-xs text-neutral-400 mt-1 font-sans">
              ESTADO ACTUAL: <strong className="text-white font-medium">{layoutConfig.automationPreset.toUpperCase()}</strong> · ÚLTIMO AJUSTE: {layoutConfig.lastUpdated}
            </p>
          </div>

          {/* Preset Buttons for 1-Click Automation */}
          <div className="flex flex-wrap items-center gap-2">
            <span className="text-xs font-sans text-neutral-400 uppercase mr-1 font-medium">
              AUTOMATIZAR EN 1 CLIC:
            </span>
            <button
              onClick={() => onAutomationApply('auto-latest')}
              className={`px-3 py-1.5 text-xs font-sans uppercase tracking-wider rounded-md flex items-center gap-1.5 transition-colors cursor-pointer border ${
      ...
      <div className="pb-8 border-b border-white/10 font-sans">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-center">
          {/* Active Preview */}
          <div className="lg:col-span-8 p-4 border border-white/10 rounded-xl flex flex-col sm:flex-row gap-4 items-start bg-neutral-950/40">
            {currentLead && (
              <>
                <div className="w-full sm:w-48 aspect-[16/9] bg-neutral-950 overflow-hidden shrink-0 rounded-lg">
                  <img src={currentLead.image} alt={currentLead.title} className="w-full h-full object-cover" />
                </div>
                <div className="flex-1 min-w-0">
                  <div className="text-xs font-sans uppercase text-neutral-400 mb-1 font-medium">
                    {currentLead.category} · {currentLead.readTime}
                  </div>
                  <h4 className="font-headline text-lg sm:text-xl font-normal text-white line-clamp-2 leading-snug">
                    {currentLead.title}
                  </h4>
          ...
          <div className="lg:col-span-4">
            ...
            <select
              ...
              className="w-full bg-black border border-white/20 rounded-md p-2.5 text-xs sm:text-sm text-white focus:outline-none focus:border-white font-sans"
            >
              {reports.map((rep) => (
      ...
      <div className="pb-8 border-b border-white/10 font-sans">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {[0, 1].map((colIndex) => {
            ...
            return (
              <div key={colIndex} className="p-4 border border-white/10 rounded-xl space-y-3 bg-neutral-950/40">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-sans uppercase tracking-wider text-white font-semibold">
                    COLUMNA #{colIndex + 1}
                  </span>
                  <span className="text-xs font-sans text-neutral-500 font-medium">BLOQUE I</span>
                </div>

                {report && (
                  <div className="space-y-2">
                    <div className="aspect-[16/9] w-full bg-neutral-950 overflow-hidden rounded-lg">
                      <img src={report.image} alt={report.title} className="w-full h-full object-cover" />
                    </div>
                    <div className="text-xs font-sans uppercase text-neutral-400 font-medium">
                      {report.category}
                    </div>
                    <h5 className="font-headline text-base font-normal text-white line-clamp-2 leading-snug">
                      {report.title}
                    </h5>
                  </div>
                )}
                ...
                <div className="pt-2 border-t border-white/5">
                  <label className="block text-xs font-sans text-neutral-400 uppercase mb-1 font-medium">
                  ...
                  <select
                    ...
                    className="w-full bg-black border border-white/15 rounded-md p-2 text-xs text-white focus:outline-none focus:border-white font-sans"
                  >
                    {reports.map((rep) => (
                      <option key={rep.id} value={rep.id}>
            ...
            return (
              <div key={colIndex} className="p-4 border border-white/10 rounded-xl space-y-3 bg-neutral-950/40">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-xs font-sans text-neutral-500 font-medium">BLOQUE II</span>
                </div>

                {report && (
                  <div className="space-y-1.5">
                    <div className="aspect-[16/9] w-full bg-neutral-950 overflow-hidden mb-2 rounded-lg">
                      <img src={report.image} alt={report.title} className="w-full h-full object-cover" />
                    </div>
                    <div className="text-xs font-sans uppercase text-neutral-400 font-medium">
                      {report.category}
                ...
                <div className="pt-2 border-t border-white/5">
                  ...
                  <select
                    ...
                    className="w-full bg-black border border-white/15 rounded-md p-1.5 text-xs text-white focus:outline-none focus:border-white font-sans"
                  >
                    {reports.map((rep) => (
                      <option key={rep.id} value={rep.id}>
      ...
      <div className="pb-8 border-b border-white/10 font-sans">
        ...
        <div className="p-4 border border-white/10 rounded-xl flex flex-col sm:flex-row items-center justify-between gap-4 bg-neutral-950/40">
          <div className="flex-1 min-w-0">
            <div className="text-xs font-sans uppercase text-neutral-400 mb-1 font-medium">
              DOSSIER SELECCIONADO:
            </div>
            <h4 className="font-headline text-base sm:text-lg font-normal text-white truncate">
          ...
          <div className="w-full sm:w-80 shrink-0">
            <select
              ...
              className="w-full bg-black border border-white/20 rounded-md p-2 text-xs text-white focus:outline-none focus:border-white font-sans"
            >
              {reports.map((rep) => (
                <option key={rep.id} value={rep.id}>
                  [{rep.category}] {rep.title.slice(0, 45)}...
      ...
      <div className="font-sans">
        ...
        <form onSubmit={handleAddFlash} className="flex flex-col sm:flex-row gap-3 mb-6">
          <input
            ...
            className="flex-1 bg-black border-b border-white/20 pb-2 text-xs sm:text-sm text-white placeholder-neutral-500 focus:outline-none focus:border-white font-sans"
          />
          <select
            value={newFlashCategory}
            onChange={(e) => setNewFlashCategory(e.target.value)}
            className="bg-black border border-white/20 rounded-md px-3 py-2 text-xs font-sans text-white focus:outline-none focus:border-white"
          >
          ...
          <button
            type="submit"
            className="px-4 py-2 bg-white text-black font-semibold text-xs uppercase tracking-wider hover:bg-neutral-200 transition-colors cursor-pointer shrink-0 rounded-md shadow-sm"
          >
            + AÑADIR A TELETIPO
          </button>
        </form>
